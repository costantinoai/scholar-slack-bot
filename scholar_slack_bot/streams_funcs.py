...
def update_cache_only(args):
    ...
    """
    logging.debug("Moving temporary cache from %s to %s", args.temp_cache_path, args.cache_path)
    confirm_temp_cache(args.temp_cache_path, args.cache_path)
    logging.info("Fetched pubs successfully moved to cache and temporary cache cleared.")
...
def test_fetch_and_message(args, ch_name, token):
    ...
    logging.debug("Running test_fetch_and_message")
    ...
    for formatted_message in formatted_messages:
        logging.debug("Sending test message to %s", ch_name)
        formatted_message = f"```\n{test_header}\n{formatted_message}\n```"
...
def regular_fetch_and_message(args, ch_name, token):
    ...
    logging.debug("Running regular_fetch_and_message")
    ...
    for formatted_message in formatted_messages:
        logging.debug("Sending message to %s", ch_name)
        response_json = send_to_slack(ch_name, formatted_message, token)
...
def refetch_and_update(args):
    ...
    logging.debug("Refetching all publications and updating cache")
    # Attempt to delete the old cache.
    if os.path.isdir(args.temp_cache_path):
        try:
            shutil.rmtree(args.cache_path)
            logging.debug("Deleted old cache at %s", args.cache_path)
        except Exception as e:  # Handle specific exception to avoid broad except.
            logging.error("Failed to delete old cache at %s. Reason: %s", args.cache_path, str(e))
...
def add_scholar_and_fetch(args):
    ...
    logging.debug("Adding new author %s", args.add_scholar_id)
    author_dict = add_new_author_to_json(args.authors_path, args.add_scholar_id)
    logging.info("Added new author with scholar ID %s", args.add_scholar_id)
    ...
    logging.debug("Converted new author's data into tuple: %s", authors)
